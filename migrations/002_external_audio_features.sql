-- SQLite migration: external (non-Reccobeats) audio-feature fill
-- Idempotent. Run after 001_audio_features.sql.

-- Staging table: raw rows ingested from a CSV (e.g. Kaggle dump).
-- One row per (source, Spotify ID); ISRC is optional metadata for fallback matching.
...
-- Per-track filled values from an external source. One row per track.
-- Source is recorded for provenance; matched_by tells you which key was used.
CREATE TABLE IF NOT EXISTS track_audio_features_external (
    track_id          TEXT PRIMARY KEY REFERENCES tracks(track_id),
    source            TEXT NOT NULL,
    matched_by        TEXT NOT NULL,        -- 'spotify_id' | 'isrc'
    acousticness      REAL,
    danceability      REAL,
    energy            REAL,
    instrumentalness  REAL,
...
-- Convenience view: prefer Reccobeats values, fall back to the external table.
-- audio_features_source records which one ended up populating the row.
CREATE VIEW IF NOT EXISTS v_track_audio_features_merged AS
SELECT
    t.track_id,
