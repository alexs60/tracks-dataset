...
def needs_scrape(conn: sqlite3.Connection, track_id: str) -> bool:
    row = conn.execute(
        "SELECT last_scraped FROM tracks WHERE track_id = ?", (track_id,)
    ).fetchone()
    if row is None or row[0] is None:
        return True
    return (datetime.utcnow() - datetime.fromisoformat(row[0])) > timedelta(days=FRESHNESS_DAYS)
...
def save_track_entries(conn: sqlite3.Connection, track_id: str, entries: list[dict]) -> None:
    ...
    conn.execute(
        "UPDATE tracks SET last_scraped = ? WHERE track_id = ?",
...
def main() -> None:
    ...
    with httpx.Client(headers={"User-Agent": USER_AGENT},
        ...
        to_scrape = [t for t in tracks if needs_scrape(conn, t["track_id"])]
        if args.limit is not None:
            to_scrape = to_scrape[: args.limit]
        print(f"Need to (re)scrape {len(to_scrape)} tracks.")
